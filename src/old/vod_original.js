...
const Vod = ({input_index}) => {
          ...
          const updateCurrentTime = () => {
            ...
            if (leftDashPlayer) {
              ...
              if (!leftDashPlayer.isPaused()){
                checkTrackingEvents(leftDashPlayer,leftAdStreams,leftCurrentStream,leftTrackingEvents,setLeftTrackingLabels,id);
              }
            }
            if (rightDashPlayer) {
              ...
              if (!rightDashPlayer.isPaused()) {
                checkTrackingEvents(rightDashPlayer,rightAdStreams,rightCurrentStream,rightTrackingEvents,setRightTrackingLabels,id);
              }
            }
          };
          
          let interval = setInterval(updateCurrentTime, 250);

          const resetInterval = () => {
            console.log('period reset');
            clearInterval(interval);
            interval = setInterval(updateCurrentTime, 250);
          };
    ...
    const checkIfAd = (period) => {
      //console.log('checkIfAd');
      //console.log(period);
      //console.log(period.duration);
      //console.log(period.EventStream);
      //console.log('ad period -> ', period['dai:adPeriod']);
      ...
      if (adStreams.includes(currentStream)) {
        ...
        for (let q = 0; q < trackingEvents.length; q++) {
          ...
          if ((trackingEvents[q].stream === currentStream) && ((player.time(manifestid)*1000) >= trackingEvents[q].pt) && (!trackingEvents[q].reported)) {
            console.log('HTTP GET: ', trackingEvents[q].url);
        ...
        <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <div>
                    ...
                    <label>Stream Type: <b>{leftStreamIsAd ? ' :: AD :: ' : '-'}</b></label><br/>
                    <label>Stream Playback Time: {formatTime(leftCurrentTime)}</label>
                    <div>
                ...
                <div>
                    ...
                    <label>Stream Type: <b>{rightStreamIsAd ? ' :: AD :: ' : '-'}</b></label><br/>
                    <label>Stream Playback Time: {formatTime(rightCurrentTime)}</label>
                    <div>
